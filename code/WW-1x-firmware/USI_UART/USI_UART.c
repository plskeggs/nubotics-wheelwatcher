...
#define PRESCALE_BITS ((0<<CS02)|(0<<CS01)|(1<<CS00))                   // Start Timer0.
#elif TIMER_PRESCALER == 8
#define PRESCALE_BITS ((0<<CS02)|(1<<CS01)|(0<<CS00))
#endif

#define USI_COUNTER_MAX_COUNT     16
#define USI_COUNTER_SEED_TRANSMIT (USI_COUNTER_MAX_COUNT - HALF_FRAME)
#define INTERRUPT_STARTUP_DELAY   (0x44 / TIMER_PRESCALER)
...
USI_UART_Initialise_Transmitter( void )                              
{
    ...
    OCR0A = 0xFF;
    OCR0B = 0xFF;
    TCCR0A = 0;                                               // Select normal mode.
    TCCR0B = PRESCALE_BITS;                                   // Start Timer0.
...
{  
    ...
    PCMSK |=  (1<<PB0);                                     // Enable only PB0 (RX/DI)
...
void 
BOOTLOADER_SECTION 
USI_UART_Transmit_Byte( unsigned char data )          
{
...
{
    return(int)USI_UART_Receive_Byte();
}
    ...
    if (USI_UART_status.ongoing_Transmission_From_Buffer)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                DDRB  &= ~(1<<PB1);                                     // Configure USI_DO as input.
                TCCR0B = (0<<CS02)|(0<<CS01)|(0<<CS00);                 // Stop Timer0.
//              PORTB |=   (1<<PB3)|(1<<PB2)|(1<<PB1)|(1<<PB0);         // Enable pull up on USI DO, DI and SCK pins. (And PB3 because of pin change interrupt)   
//              DDRB  &= ~((1<<PB3)|(1<<PB2)|(1<<PB1)|(1<<PB0));        // Set USI DI, DO and SCK pins as inputs.  
                USICR  =  0;                                            // Disable USI.
//              GIFR   =  (1<<PCIF);                                    // DON'T OR WE COULD MISS INTERRUPTS: Clear pin change interrupt flag.
                PCMSK |=  (1<<PB0);                                     // Enable only PB0 (RX/DI)
                GIMSK |=  (1<<PCIE);                                    // Enable pin change interrupt for PB3:0.
            }
        }
    ...
    else
    {
        ...
        else                                                    // If there is space in the buffer then store the data.
        {
            ...
            UART_RxBuf[tmphead] = USIBR;                        // Store received data in buffer. Note that the data must be bit reversed before used. 
#ifdef FULL_COMMAND_HANDLER
            ctrl.flags.process_cmd = handle_serial_received_byte(ctrl.flags.process_cmd);
#endif
        }                                                       // The bit reversing is moved to the application section to save time within the interrupt.
